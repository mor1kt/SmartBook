...
export default function AdminIndividualBookingsPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        if (!accessToken) {
          navigate(`/login?next=${encodeURIComponent(`/c/${slug}/admin/bookings/individual`)}`, { replace: true });
          return;
        }
        if (!mounted) return;
    ...
    <div className="al-root">
      ...
      <div className="al-shell">
        ...
        <main className="al-main">
          <h1 className="al-title">Записи на индивидуальные занятия</h1>
          {loading ? <div className="al-empty">Загрузка…</div> : null}
          {error ? <div className="al-empty" style={{ color: '#b91c1c' }}>{error}</div> : null}

          <div className="al-list">
            {!loading && individualBookings.length === 0 ? <div className="al-empty">Пока нет записей.</div> : null}
