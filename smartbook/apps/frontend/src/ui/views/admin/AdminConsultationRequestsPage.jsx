...
function parseSlotMessage(message) {
  ...
  if (!a || !b) return null;
  return { date: a.slice(0, 10), start: a.slice(11, 16), end: b.slice(11, 16) };
}
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        if (!accessToken) {
          navigate(`/login?next=${encodeURIComponent(`/c/${slug}/admin/consultations`)}`, { replace: true });
          return;
        }
        if (!mounted) return;
    ...
    <div className="al-root">
      ...
      <div className="al-shell">
        ...
        <main className="al-main">
          <h1 className="al-title">Записи на индивидуальные консультации</h1>
              ...
              return (
                <div className="al-item" key={w.id}>
                  <div className="al-item-top">
                    <div>
                      <div className="al-item-title">{w?.student_name || 'Студент'}</div>
                      <div className="al-item-meta">
                        {w?.student_phone ? `Тел: ${w.student_phone}` : null}
                        {w?.created_at ? ` • ${formatDate(w.created_at)}` : null}
                        {slot ? ` • ${slot.date} ${slot.start}-${slot.end}` : null}
                      </div>
                    </div>
