...
export default async function handler(req: any, res: any) {
  ...
  try {
    ...
    if (courseError || !course) return sendJson(res, 404, { error: 'Course not found in this center' });
    if ((course as any).booking_type !== 'group') return sendJson(res, 400, { error: 'Invalid course type' });
